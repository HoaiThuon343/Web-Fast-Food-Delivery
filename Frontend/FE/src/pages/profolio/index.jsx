...
export default function ProfilePage() {
  ...
  const AccountUser = useSelector((store) => store.account);
  ...
  const fetchUserId = async () => {
    ...
    const response = await axios.get(
      `http://localhost:5213/api/User/GetUserById/${AccountUser.UserId}`
    );
  ...
  async function handleEditAccount() {
    ...
    await axios.put(
      `http://localhost:5213/api/User/UpdateUser?id=${AccountUser.UserId}`,
      updateAccount
    );
